...
- 所以常量在声明时候没有默认值，这就要求程序在声明常量的时候必须指定该常量的值。Ti


#### 对象的上转型对象：

“老虎是动物 ， 狗是动物” ， 但是说老虎是动物的时候，老虎将失去 老虎独有的属性和功能 

- 从思维上来看，老虎是动物属于 上溯思维模式！

```
Animal a;
Tiger b = new Tiger();
a = b ; // a对象  是 b 对象的一个上转型对象
```
上转型对象可以理解为一个简化后的子类对象
- 上转型对象 不能 操作 子类新增的成员变量（失去这部分属性）不能调用新增的方法
- 上转型对象可以访问子类继承隐藏的成员变量，也可以调用子类继承的方法或子类重写的实例方法 。（ 其作用方法等价于子类对象区调用这些方法）

注意！:
- 不要将父类创建的对象 和 子类对象的上转型对象混淆
- 可以将对象的上转型对象再强制转换成一个子类对象，这时子类对象又具备了子类所有的属性和功能
- 不可以将父类创建的对象的引用赋值给子类声明对象（不能说 人  “美国人”）
