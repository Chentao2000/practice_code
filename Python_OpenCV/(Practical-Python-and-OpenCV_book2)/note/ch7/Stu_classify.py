...
# grab the image and masks paths

imagePaths = sorted(glob.glob(args["images"] + "/*.png" ))
masksPaths = sorted(glob.glob(args["masks"] + "/*.png" ))
...
# loop over the image and masks targets

for (imagePath ,maskPath) in zip ( imagePaths , masksPaths):
# load the images  and masks paths
...
# . fit : -> Any Build a forest of trees from the training set
